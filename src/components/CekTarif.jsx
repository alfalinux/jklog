...
import { MapPinIcon, ShoppingCartIcon, ChevronUpDownIcon, CheckIcon } from "@heroicons/react/24/solid";
...
const CekTarif = () => {
  const sortedDataTujuan = [...dataTujuan].sort((a, b) => a.kota.localeCompare(b.kota));
  const [asal, setAsal] = useState("Surabaya");
  ...
  const tujuanChangeHandler = (e) => {
    ...
    setGetDataTujuan(dataTujuan.find((item) => item.kota === e.target.value));
  };
  ...
  const getSelectedTujuan = (kota) => {
    return dataTujuan.find((item) => item.kota === kota) || null;
  };
  ...
  return (
    <>
      ...
      >
        <form className="space-y-4 md:flex items-center justify-center md:space-x-4">
          {/* Asal */}
          <div className="relative w-full mt-4">
            ...
            <div className="relative mt-1">
              <select
                ...
                className="block w-full pl-3 pr-3 py-2 border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-biru-muda-jkl focus:border-biru-muda-jkl sm:text-sm"
              >
                <option value="">Silahkan Pilih Kota Asal</option>
          ...
          <div className="relative w-full mt-4">
            ...
            <div className="relative mt-1">
              ...
              >
                ...
                {sortedDataTujuan.map((item) => (
                  <option key={item.id} value={item.kota}>
                    {item.kota}
                  </option>
                ))}
              </select>
          ...
          <div>
            <button
              type="submit"
              className="w-full bg-gray-800 hover:bg-gray-900 text-white py-2 px-4 mt-6 rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-biru-muda-jkl"
              onClick={handleSubmit}
              style={{ whiteSpace: "nowrap" }} // Tambahkan whiteSpace: 'nowrap'
            >
              Cek Tarif
            </button>
          </div>
